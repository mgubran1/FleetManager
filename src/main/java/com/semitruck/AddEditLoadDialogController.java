...
public class AddEditLoadDialogController {
    ...
    /**
     * Fix: Always set the correct driverId from employeeList for the selected driver name in ComboBox.
     */
    public Load getLoadFromFields() {
        ...
        return new Load(
            loadIdField.getText(),
            driverId, // <-- FIX: always use driverId that matches employees table!
            selectedDriverName,
            pickupDatePicker.getValue(),
            deliveryDatePicker.getValue(),
            null, // pickup_address (can add more fields if you collect them in the dialog)
    ...
    public void handleOk() {
        if (isInputValid()) {
            if (load != null) {
                ...
                load.setDriverId(fromFields.getDriverId()); // <-- ensure driverId is set
